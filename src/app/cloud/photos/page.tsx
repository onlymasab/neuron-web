...
const styles = {
  primaryColor: "#1E40AF", // Darker blue for better contrast
...
const Photos = () => {
  ...
  // Progress animation logic (unchanged)
  useEffect(() => {
    ...
    progressInterval = setTimeout(() => {
      progressInterval = setInterval(() => {
        ...
        const percent = Math.min((elapsed / 10000) * 90, 90);
  ...
  // handleConvert function (unchanged for brevity)
  ...
  return (
    <div
      className="flex flex-col min-h-screen p-6 bg-gray-50"
      style={{ backgroundColor: styles.backgroundColor }}
    >
      <Tabs defaultValue="gallery" className="w-full">
        <TabsList
          className="flex justify-start gap-4 p-4 bg-transparent border-b border-gray-200"
          style={{ paddingBottom: styles.spacing.md }}
        >
          {["gallery", "album", "explore"].map((value) => (
            <TabsTrigger
              key={value}
              value={value}
              className="px-4 py-2 text-base font-medium capitalize rounded-md transition-all duration-200 data-[state=active]:bg-gradient-to-r data-[state=active]:from-blue-500 data-[state=active]:to-blue-700 data-[state=active]:text-white hover:bg-gray-100 focus:outline-none focus:ring-2 focus:ring-blue-500"
            ...
            {isLoading ? (
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                {recentImages.map((image, index) => (
                  <div
                    key={index}
                    className="relative flex flex-col bg-white rounded-lg shadow-md overflow-hidden transition-transform duration-200 hover:scale-105"
                    style={{ boxShadow: styles.shadow }}
                  >
                    <div className="flex justify-between items-center p-2 bg-gray-50">
                      <span className="text-sm text-gray-600">{image.date}</span>
                      <DropdownMenu>
                        <DropdownMenuTrigger asChild>
                          <Button
                            variant="ghost"
                            className="h-8 w-8 p-0 hover:bg-gray-100 focus:ring-2 focus:ring-blue-500"
                        ...
                        <DropdownMenuContent align="end">
                          ...
                          <DropdownMenuSub>
                            ...
                            <DropdownMenuPortal>
                              <DropdownMenuSubContent>
                                <DropdownMenuItem onClick={() => handleConvert("dna", image.src)}>
                                  DNA
                                </DropdownMenuItem>
                                <DropdownMenuItem onClick={() => handleConvert("graphene", image.src)}>
                                  Graphene
                                </DropdownMenuItem>
                                <DropdownMenuItem onClick={() => handleConvert("brain", image.src)}>
                                  Brain Signals
                                </DropdownMenuItem>
                              </DropdownMenuSubContent>
                            </DropdownMenuPortal>
                          ...
                          <DropdownMenuItem
                            onClick={async () => {
                              try {
                                await useCloudStore.getState().deleteFile(image.id);
                                useCloudStore.setState((state) => ({
                                  files: state.files.filter((f) => f.id !== image.id),
                                }));
                                toast.success(`${image.throwback} moved to trash`, {
                                  icon: "🗑️",
                                });
                              } catch (error: any) {
                                toast.error(`Failed to delete ${image.throwback}: ${error.message}`, {
                                  icon: "❌",
                                });
                              }
                            }}
                    ...
                    <Image
                      src={image.src}
                      alt={`Photo uploaded on ${image.throwback}`}
                      width={296}
                      height={300}
                      className="w-full h-48 object-cover"
                      placeholder="blur"
                      blurDataURL="/images/placeholder.jpg" // Add a placeholder image
                    />
                  </div>
                ))}
              </div>
