...
interface CloudStore {
  ...
  fetchFilesWithEmails: () => Promise<void>;
...
export const useCloudStore = create<CloudStore>((set, get) => {
  ...
  return {
    ...
    fetchLikedFiles: async () => {
      ...
      const { data, error } = await supabase.from("liked_files").select("*");
    ...
    createFolder: async (folderName, parentId) => {
      ...
      const url = supabase.storage.from("cloud").getPublicUrl(data.path)
        .data.publicUrl;
      meta.file_url = url;
